...
（watchlist に無いページを候補として検出し、分類キューに積む）
...
出力:
  reports/discover_YYYY-MM-DD.md … 人が読むレポート（該当箇所つき）
  data/discovery/queue.json      … 分類層(classify_prepare.py)が読むキュー

モード:
  通常              : python scripts/discover.py [--pref 東京都] [--limit 5]
  sitemap有無の全数調査: python scripts/discover.py --probe

設計思想（不変）: 発見から分類・掲載まで全自動で回す（対象は全国1,741自治体、
人が1件ずつ承認する運用ではスケールしない）。精度は人のレビューではなく構造で守る:
入力は公式一次情報（自治体公式ドメイン）のみ、判定には逐語引用を必須とし、
引用が取れないものは禁止と断定せず unknown に倒す（classify_apply.py が機械検証する）。
...
# 発見の必須条件: ドローンを指す語が1つ以上あること。
# ・「行為許可」「撮影許可」等はドローン非言及の公園ページにも頻出するため単独では不可。
# ・「ラジコン」系は一切採用しない。今どきドローンを「ラジコン飛行機」とは書かないうえ、
#   地上を走るラジコン（例: 横浜市の遊歩道「ラジコンやスケートボードなどの走行」）を
#   誤検知する。ドローンと書いていないならドローンの話ではない。
# ・「無人航空機」「小型無人機」等は残す。航空法・小型無人機等飛行禁止法における
#   ドローンの正式名称で、自治体公式が実際にこの表記を使うため（座間・警視庁など）。
...
def main():
    ...
    n_queued = enqueue(discoveries)
    print(f"対象{len(targets)} / 発見{len(discoveries)} / ベースライン{len(baselines)} / "
          f"sitemap無し{len(no_sitemap)}(うちクロール{len(crawled)}) / キュー投入{n_queued}",
          file=sys.stderr)
    sys.exit(1 if discoveries else 0)


def enqueue(discoveries):
    """発見を分類キュー(data/discovery/queue.json)に積む。

    レポート(md)は人が読む用。分類層はこのJSONを読む。
    URLで重複排除するので、同じページを何度発見しても二重に積まれない。
    分類が済んだものは classify_apply.py が pending から取り除く。
    """
    q = {"pending": []}
    if QUEUE.exists():
        try:
            ...
            pass
    known_urls = {i.get("url") for i in q.get("pending", [])}
    added = 0
    for pref, name, url, hits, snip, src in discoveries:
        if url in known_urls:
            continue
        known_urls.add(url)
        q["pending"].append({"pref": pref, "muni": name, "url": url,
                             "hits": hits, "snippet": snip, "via": src,
                             "found": TODAY})
        added += 1
    if added:
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if discoveries:
            ...
            for pref, name, url, hits, snip, src in discoveries:
                ...
                if snip:
                    f.write(f"- 該当箇所: {snip}\n")
                f.write("- → 分類キューに投入済み。classify で逐語確認のうえ regulations に反映\n\n")
